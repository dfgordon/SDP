...
80  REM PROMPT
81  HOME: VTAB 21: PRINT "MOVE=Y,G,J,SPC TOGGLE=Z UNDO=U DONE=Q": PRINT "PEN IS ";PEN$: RETURN
...
303  IF V$ = "S" AND ST = 0 THEN ST = 1: X(0) = X(MV): Y(0) = Y(MV): GOSUB 80: GOTO 300
...
312  IF V$ = "U" AND MV > 0 AND ST = 1 THEN GOSUB 450: GOTO 300
...
450 REM UNDO
451 HCOLOR = 0: GOSUB 440: FOR I = MV - 1 TO 0 STEP -1: IF P(I) THEN DRAW 2 AT X(I),Y(I)
452 NEXT: HCOLOR = 3: POKE ADDR,0: GOSUB 440: RETURN
...
1100  REM LIST
1101  LN = 0
1110  TEXT: HOME: I = FN SA(VN): PRINT "BITS HIGH -> LOW:": GOTO 1120
1115  IF LN > 22 THEN PRINT "MORE? ";: GET A$: LN = 0: PRINT: IF A$="N" THEN POKE -16304,0: RETURN
1120  FOR J = 7 TO 0 STEP -1: M = INT(PEEK(I)/(2^J)): M = M - 2*INT(M/2): PRINT M;: IF J=6 OR J=3 THEN PRINT " ";
1130  NEXT: PRINT " (";PEEK(I);")": IF PEEK(I)>0 THEN I = I + 1: LN = LN + 1: GOTO 1115
1140  PRINT "DONE ";: GET A$: POKE -16304,0: RETURN
